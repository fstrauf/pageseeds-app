...
pub fn discover_sources(project_path: &Path, config: &SourceConfig) -> Result<SourceManifest> {
...
fn discover_articles(
    ...
    log::info!(
        "[discover_articles] content resolution: source={}, has_markdown={}, selected={:?}",
        resolution.source,
        resolution.has_markdown,
        resolution.selected
    );
    ...
    log::info!(
        "[discover_articles] checked {} dirs, found {} existing dirs, {} articles total",
        checked_dirs,
        found_dirs,
        articles.len()
    );
...
fn discover_screenshots(
    ...
    log::info!(
        "[discover_screenshots] checking {} dirs",
        dirs_to_search.len()
    );
    ...
    for dir in &dirs_to_search {
        ...
        let entries = std::fs::read_dir(&dir)?.filter_map(|e| e.ok()).filter(|e| {
            let path = e.path();
            let ext = path.extension().and_then(|e| e.to_str());
    ...
    log::info!(
        "[discover_screenshots] checked {} dirs, found {} existing dirs, {} screenshots total",
        checked_screenshot_dirs,
        found_screenshot_dirs,
        screenshots.len()
    );
    Ok(screenshots)
}
