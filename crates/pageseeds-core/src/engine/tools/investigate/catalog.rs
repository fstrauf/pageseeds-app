//! Tool catalog loaded from `crates/pageseeds-core/config/tool_catalog.toml`.
//!
...
mod tests {
    ...
    fn catalog_parses_all_expected_tools() {
        ...
        assert_eq!(names.len(), 22, "expected 22 tools in catalog, got {names:?}");
        for required in [
            "gsc_performance",
            "gsc_queries",
            "gsc_movers",
    ...
    fn catalog_mutates_flags() {
        ...
        assert_eq!(catalog_names_for_access(InvestigationAccess::Full).len(), 22);
        assert_eq!(
