...
pub async fn ensure_runner_started(
    ...
    tokio::spawn(async move {
        run_queue(db_path, gsc_token).await;
        RUNNER_ACTIVE.store(false, Ordering::SeqCst);
        log::info!("[queue] Runner finished");
    });
}

async fn run_queue(db_path: PathBuf, gsc_token: Option<String>) {
        ...
        match result {
            Ok(Ok(exec_result)) => {
                ...
                if exec_result.success {
                    ...
                    .ok();
                    emit_completed(&item, &exec_result);
                    ...
                    for follow_up in &exec_result.follow_up_tasks {
                        if follow_up.run_policy == "auto_enqueue" {
                                ...
                                enqueue_tasks(&conn, vec![enqueue_item], EnqueueMode::Append)
                            {
                                ...
                                emit_follow_up(&item.project_id, follow_up);
                            }
                        }
                    }
                } else {
                    ...
                    emit_failed(&item, &exec_result.message);
...
// ─── Event Emission (log-only; desktop IPC deferred to #184) ─────────────────

fn emit_started(item: &QueueItem) {
    log::info!(
        "[queue] queue:task-started task_id={} project_id={} title={:?} type={:?}",
        item.task_id,
        item.project_id,
        item.title,
        item.task_type
...
fn emit_completed(item: &QueueItem, result: &ExecutionResult) {
    log::info!(
        "[queue] queue:task-completed task_id={} success={} message={}",
        item.task_id,
        result.success,
        result.message
    );
...
fn emit_failed(item: &QueueItem, error: &str) {
    log::info!(
        "[queue] queue:task-failed task_id={} error={}",
        item.task_id,
        error
    );
}
...
fn emit_follow_up(project_id: &str, follow_up: &executor::FollowUpTask) {
    log::info!(
        "[queue] queue:follow-up-created task_id={} project_id={} type={} policy={}",
        follow_up.id,
        project_id,
        follow_up.task_type,
