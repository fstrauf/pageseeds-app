...
mod tests {
    ...
    fn test_investigation_read_only_tools_excludes_mutators() {
        ...
        assert_eq!(full.len(), 22, "full set must remain 22 tools");

        let ro = investigation_read_only_tools(ctx);
        assert_eq!(ro.len(), 18, "read-only set must be 18 tools");
    ...
    fn test_kit_catalog_and_tools_aligned_per_mode() {
        ...
        assert_eq!(inventory_names(InvestigationAccess::Full).len(), 22);
        assert_eq!(inventory_names(InvestigationAccess::ReadOnly).len(), 18);
    }
