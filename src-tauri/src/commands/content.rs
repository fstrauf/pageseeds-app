...
pub fn repair_article_paths(
    ...
    let project = task_store::get_project(&db, &project_id).map_err(|e| e.to_string())?;
    let repo_root = std::path::Path::new(&project.path);
    crate::content::article_resolver::repair_article_paths_in_batch(repo_root, &project_id, &db)
        .map_err(|e| e.to_string())
}
...
pub fn get_ctr_health_summary(
    ...
    let completed_audits = all_tasks
        .iter()
        .filter(|t| t.task_type == "ctr_audit" && t.status == crate::models::task::TaskStatus::Done)
        .count();
