...
fn setup_test_project(dir: &Path) {
    ...
    std::fs::write(automation_dir.join("project_summary.md"), TEST_PROJECT_SUMMARY)
...
async fn test_full_queue_flow_with_real_execution() {
    ...
    let backend_logs: Vec<_> = all_logs.iter()
        .filter(|l| matches!(l.source, LogSource::Backend))
        .collect();
    let frontend_logs: Vec<_> = all_logs.iter()
    ...
    let stats = pageseeds_lib::logging::get_log_stats(&conn)
        .expect("Failed to get log stats");
