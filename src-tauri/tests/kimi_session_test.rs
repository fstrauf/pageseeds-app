...
{"product_name": "Test Product", "mention_stance": "OPTIONAL", "trigger_topics": ["test"], "query_keywords": ["test"], "seed_subreddits": ["test"], "excluded_subreddits": []}"#;
    ...
    for i in 1..=3 {
        ...
        let output =
            pageseeds_lib::engine::agent::run_agent("kimi", prompt, Path::new(project_path))
        ...
        let parsed: serde_json::Value =
            serde_json::from_str(&json_str).expect(&format!("Should parse JSON from call {}", i));

        assert_eq!(
            parsed.get("product_name").and_then(|v| v.as_str()),
            Some("Test Product"),
            "Call {}: product_name should match",
            i
        );
    ...
    println!(
        "\n✅ All {} calls returned consistent-sized output",
        output_sizes.len()
    );
...
fn test_kimi_reddit_config_parsing_multiple_times() {
    ...
    for (i, size) in sizes.iter().enumerate() {
        ...
        if deviation > 0.5 {
            panic!(
                "Run {} output size ({} bytes) deviates too much from average ({:.0} bytes)",
                i + 1,
                size,
                avg_size
            );
    ...
    println!(
        "\n✅ All {} runs succeeded with consistent output",
        results.len()
    );
}
