...
| `research_keywords` | (adds self to follow-up list for UI) | For keyword selection workflow |

**Idempotency:** All follow-up creation goes through `TaskSpawner::spawn_follow_up()` which uses deterministic keys to prevent duplicates.
