...
| `ContentReviewInvestigate` | RO tool-calling investigation (falls back to recommend) | Path-local: `investigation_findings` (tool) or `content_review_recommend` (fallback) | Hybrid: tool agent + typed Extractor; recommend fallback writes recommendations.json |
...
1. `content_write_stage` → agentic: file-IO host (`grok`/`kimi`) writes the MDX file directly into the repo (skill: `content-write` or `hub-write`). New-article tasks get an exact target path directive. Text-only providers are rejected by the issue #143 host gate before the agent runs.
2. `content_write_verify` (`ContentWriteVerify`, new-article tasks only) → deterministic: fails the task when the write stage produced no registered article file (issue #13 contract — never Done with zero output).
...
- Step 4: `ContentReviewInvestigate` — multi-turn RO tool agent when the backend
  ...
  to scripted `content_review_recommend`.
  - **Tool path:** typed `InvestigationFindings` under artifact key
    `investigation_findings` (and optional `investigation_findings.json`). Does
    **not** write `recommendations.json`. No auto fix spawn until issue #81
    wires proposed_tasks validation / picker (lifecycle metadata intentionally
    unchanged).
  - **Fallback path:** `content_review_recommend` output under artifact key
    `content_review_recommend`; still writes `recommendations.json` so
    BackendAuto `create_fix_content_article_tasks` works as today.
  - Path-local keys are set on `StepResult.artifact_key` (not a fixed plan
    `artifact_name`) so the two schemas never share one artifact key.
...
| `"artifact_name"` | any | Names the output artifact persisted to SQLite (overridden by `StepResult.artifact_key` when set) |
...
| `content_review` | `fix_content_article` tasks (one per article) | Reads `recommendations.json` and creates individual tasks — **recommend fallback only** (see dual path below) |

**Content Review Auto-Creation (dual path):**
`content_review` judgment is `ContentReviewInvestigate` with a recommend fallback:

| Path | When | Artifacts | Auto-spawn fixes |
|------|------|-----------|------------------|
| Tool-capable | KimiBridge / Claude / OpenAI / Ollama | `investigation_findings` artifact (+ optional `investigation_findings.json`); **no** `recommendations.json` | No — empty BackendAuto follow-ups until #81 proposed_tasks picker |
| Fallback | Backends without tool calling (e.g. KimiCli) | `content_review_recommend` artifact + `recommendations.json` | Yes — same as historical recommend path |

When the **fallback** path completes successfully, the system automatically:
1. Reads `recommendations.json` (generated by the recommend fallback)
...
On the **tool** path, `create_fix_content_article_tasks` no-ops (no `recommendations.json`). That is intentional for issue #80; issue #81 owns proposal validation / user picker. Do not change `task_definitions` follow_up_policy / review_surface here.
